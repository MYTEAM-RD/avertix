...
import uuid
from auth import create_user, authenticate_user, create_session, get_current_user, destroy_session, require_auth
...
STATIC_VIDEOS_DIR = "/home/ubuntu/violence_detection/violence_detection/Avertix_Auth/static_videos"
...
app.mount("/assets", StaticFiles(directory="/home/ubuntu/violence_detection/violence_detection/Avertix_Auth/frontend/assets"), name="assets")
...
class Controller():
    ...
    def start(self, config: StartUpConfig):
        ...
        if 'youtube.com' in source_url or 'youtu.be' in source_url:
            ...
            try:
                COOKIES_FILE_PATH = "/home/ubuntu/violence_detection/violence_detection/Avertix_Auth/cookies.txt"
...
async def register(user_data: UserRegister):
    try:
        ...
        return JSONResponse({"success": True, "user": user, "message": "User registered successfully"})
...
async def login(user_data: UserLogin):
    try:
        ...
        if user:
            ...
            response = JSONResponse({"success": True, "user": user, "message": "Login successful"})
            response.set_cookie(
                key="session_token", 
                value=session_token,
...
async def logout(request: Request):
    try:
        ...
        return JSONResponse(status_code=500, content={"success": False, "message": "Logout failed"})
...
async def auth_status(request: Request):
    try:
        ...
        return JSONResponse(status_code=500, content={"authenticated": False, "user": None})
# ==============================================================================

@app.get("/")
def serve_index():
    return FileResponse("/home/ubuntu/violence_detection/violence_detection/Avertix_Auth/frontend/MergedAlert.html")
...
async def analyze_youtube_offline(data: YouTubeURL):
    """Analyze a YouTube video offline by downloading it first."""
    try:
        ydl_opts = {
            'format': 'best[ext=mp4]/best',
            'outtmpl': f'{STATIC_VIDEOS_DIR}/%(title)s.%(ext)s',
            'cookiefile': '/home/ubuntu/violence_detection/violence_detection/Avertix_Auth/cookies.txt' if os.path.exists('/home/ubuntu/violence_detection/violence_detection/Avertix_Auth/cookies.txt') else None
        }
        
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info_dict = ydl.extract_info(data.url, download=True)
            video_path = ydl.prepare_filename(info_dict)
            
            result = analyze_video_file(video_path)
            
            # Clean up the downloaded file after analysis
...
# AWS endpoints commented out for testing
# @app.post("/kinesis-stream/")
# async def get_kinesis_stream_url(request: Request):
#     return JSONResponse({"error": "Kinesis streaming not available in test mode"})

# @app.get("/proxy/")
# async def proxy(url: str):
#     return JSONResponse({"error": "Proxy not available in test mode"})
